...
export default function InvoicesContent() {
  ...
  // Auto-generate monthly invoices on first load
  useEffect(() => {
    if (loading || hasGenerated.current) return
    hasGenerated.current = true
    generateMonthlyInvoices()
      .then((count) => {
        if (count > 0) {
          toast({
            title: t("invoices.autoGenerated"),
            description: `${count} ${t("invoices.autoGeneratedDesc")}`,
          })
          refetch()
        }
      })
  ...
  const handleExportFormat = (format: ExportFormat) => {
    ...
    downloadExport(format, {
      ...
      filename: `invoices${monthFilter !== "all" ? `-${monthFilter}` : ""}`,
    })
  }
